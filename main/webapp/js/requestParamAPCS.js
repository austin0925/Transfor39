 ...
 * 限定輸入大寫字母
 */
$.fn.inputUpperLimiter = function inputUpperLimiter(){
	$(this).keypress(function(e){
		var oldValue = $(this).val();
		//(e.keyCode) A->65, z->90, a->97, z->122, 0->48, 9->57
		if(e.keyCode>=65 && e.keyCode<=90){
			//正常
		}else if(e.keyCode>=97 && e.keyCode<=122){
			//正常
		}else if(e.keyCode>=96 && e.keyCode<=105){
			$(this).val(oldValue);//復原字串處理
			showStatusMsg("ng", "請輸入字母");
			return false;
		}
	});
	$(this).keyup(function(e){
		var selectionStart = e.srcElement.selectionStart;
		var oldValue = $(this).val();
		oldValue = oldValue.toLocaleUpperCase();
		$(this).val(oldValue);
		e.srcElement.setSelectionRange(selectionStart, selectionStart);
	});
};
